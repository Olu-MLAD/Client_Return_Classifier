    ...
    with st.spinner("Loading model explainability..."):
        ...
        try:
            ...
            **Interpreting the SHAP values**:
            - Features are ordered by their importance to the model
            - The length shows how much each feature affects the prediction
            - Blue bars indicate features that increase return probability
            - Red bars indicate features that decrease return probability
            """)
            
            # Force plot for individual prediction explanation
            st.markdown("### Individual Prediction Explanation")
            st.markdown("This shows how each feature contributes to a single prediction:")
            ...
            st.error(f"Could not generate SHAP explanations: {str(e)}")
            st.info("""
            For this demo, we're showing example SHAP values. In a production environment:
            1. We would use your actual trained model
            2. Compute SHAP values on representative data
